...
public class PartyManager {
    ...
    public void disband(Party party) {
        party.getMembers().forEach(member -> {
            ...
            if (player != null) {
                player.sendMessage(ChatUtil.color(plugin.getMessageManager().getString("party.disband")));
            }
        });
    ...
    public void invite(Player inviter, Player target, Party party) {
        ...
        String message = plugin.getMessageManager().getString("party.invite")
            .replace("%party%", party.getName())
            .replace("%player%", inviter.getName());
        target.sendMessage(ChatUtil.color(message));
    }
    ...
    public void setPartyMode(Party party, PartyMode mode) {
        party.setMode(mode);
        String message = plugin.getMessageManager().getString("party.modeSet")
